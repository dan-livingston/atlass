...
export function parseUpdateSource(content: string): UpdateSource {
	...
	if (!match) {
		throw new Error("Not an atlass page file: no YAML frontmatter found.");
	}
...
// would drop. Keyed by label so an update can warn "panel, 2 macros".
export function findLossyNodes(node: AdfNode | null | undefined): Map<string, number> {
