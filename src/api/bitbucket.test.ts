...
import { pipelinesQuery, pipelineStatus, stepDurationSeconds } from "./bitbucket.ts";
...
test("stepDuration: elapsed whole seconds between start and completion", () => {
	expect(stepDurationSeconds("2026-07-17T12:00:00Z", "2026-07-17T12:01:10Z")).toBe(70);
});

test("stepDuration: a step still running has no duration", () => {
	expect(stepDurationSeconds("2026-07-17T12:00:00Z", undefined)).toBeNull();
	expect(stepDurationSeconds(undefined, undefined)).toBeNull();
});
